...
namespace patterns
{
	...
	namespace shared
	{
		...
		PATTERNS(MsgFunc_TextMsg,
		   "HL-SteamPipe-Linux",
		   "55 31 D2 57 BF 01 00 00 00"
		   //"56 53 83 EC 24 8B 44 24 ?? 89 44 24 ?? 8B 44 24 ?? 89 04 24 E8 ?? ?? ?? ?? E8 ?? ?? ?? ?? 89 C3 E8 ?? ?? ?? ?? 85 C0 89 C6 0F 84 ?? ?? ?? ?? 80 38 23 0F 84 ?? ?? ?? ?? B8 80 00 00 00"
		   //"56 53 83 EC 24 8B 44 24 ?? 89 44 24 ?? 8B 44 24 ?? 89 04 24 E8 ?? ?? ?? ?? E8 ?? ?? ?? ?? 89 C3 E8 ?? ?? ?? ?? 85 C0 89 C6 0F 84 ?? ?? ?? ?? 80 38 23 0F 84 ?? ?? ?? ?? B8 80 00 00 00"
		  // "56 53 83 EC 24 8B 44 24 ?? 89 44 24 ?? 8B 44 24 ?? 89 04 24 E8 ?? ?? ?? ?? E8 ?? ?? ?? ?? 89 C3 E8 ?? ?? ?? ?? 85 C0 89 C6 0F 84 ?? ?? ?? ?? 80 38 23 0F 84 ?? ?? ?? ?? B9 80 00 00 00"
		   //"55 57 56 53 83 EC 1C 8B 44 24 ?? 89 44 24 ??"
		   //"53 83 EC 18 8B 44 24 ?? 8B 5C 24 ?? 89 44 24 ?? 8B 44 24 ?? 89 04 24 E8 ?? ?? ?? ?? E8 ?? ?? ?? ?? 8B 15 ?? ?? ?? ??"
		   //"56 53 83 EC 24 8B 44 24 ?? 89 44 24 ?? 8B 44 24 ?? 89 04 24 E8 ?? ?? ?? ??"
		);
